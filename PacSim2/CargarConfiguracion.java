		...
		while(sc.hasNextLine() && !line.equals("[MINAS]")) {
			...
			while(st.hasMoreTokens()) {
				...
				if(actual.equals("E")) {
					...
					tablero.agregarPac(new Pac(posP, fila, columna, dimC), posP);
				}
				if(actual.equals("S")) {
					int posS = Integer.parseInt(st.nextToken()); 
					tablero.agregarPosicionable(new Salida(), posS);
				}
				if(actual.equals("L")) {
					...
					for(int i=0 ; i < cadena.length() ; i++) {
						...
						if(chars == '1') {
							tablero.agregarPosicionable(new Pared(),i);
						}
					}
				}
			}
				...
				while(st.hasMoreTokens()) {
					...
					if(actual.equals("B")) {
						...
						while(stAux.hasMoreTokens()) {
							...
							tablero.agregarPosicionable(new Mina(),actualAux);
						}
					}
				}
			}
				...
				if(st.countTokens() == 1) {
					...
					while(stAux.hasMoreTokens()) {
						...
						if(actualAux.equals("E")) {
							...
							dicc.get(actual).agregar(new CajaDeVitaminas());
						} else {
			...
			while(st.hasMoreTokens()){
				...
				if(st.countTokens() == 1) { 
					...
					while(stAux.hasMoreTokens()) {
						...
						tablero.agregarPosicionable(dicc.get(actual),pos);
					}		
				}
			}
		}
